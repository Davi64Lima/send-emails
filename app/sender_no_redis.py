...
class Sender(Bottle):
    ...
    def register_message(self, assunto, mensagem, email):
        ...
        # Simulate queue message (without Redis)
        msg = {'data': now.isoformat(), 'assunto': assunto, 'mensagem': mensagem, 'email': email}
        print(f"[QUEUE SIMULATION] Message would be sent to queue: {json.dumps(msg)}")
        print('[OK] Mensagem registrada! (Redis desabilitado)')
    ...
    def send(self):
        try:
            ...
            success_msg = f'Mensagem registrada! Assunto: {assunto} Mensagem: {mensagem} Email: {email}'
